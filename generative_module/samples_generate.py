...
def samples_generate():
    ...
    # The generator neural network is initialized with the specified hyperparameters
    ...
    # Random noise is generated with the specified latent_dim
    z = torch.randn(args.samples_size, args.latent_dim).to(device)

    batch_size = 200
    samples = torch.Tensor([])
    samples_label = torch.LongTensor([])
    # The generator is called once to generate all samples.
    ...
    # The generator is called multiple times with smaller batch sizes.
    else:
    ...
    # The generated samples are normalized and rescaled to match the range of the training data
