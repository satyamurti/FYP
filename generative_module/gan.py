...
def train():
    ...
    # Data preprocessed by normalizing it between 0 and 1 using min max scaling
    max=[]
    min=[]
    ...
    # Shape and Size of the dataset are stored in the variables
    samples_size = data.shape[0]
    series_len = data.shape[2]*data.shape[3]
    series_dim = data.shape[1]

    # Initializing Generator and Discriminator networks using the specified hyperparameters
    ...
    # Optimizer for both Generator and Discriminator is defined as Adam Optimizer
    ...
    # Data is converted into PyTorch tensor
    data = torch.Tensor(data)
    dataset = torch.utils.data.TensorDataset(data,torch.ones(samples_size))

    # Data Loader is created to load data into batches
    data_loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=args.batch_size, shuffle=True)
    print('Printing architecture of both Generative and Discriminator networks')
    print('##network structure discription##')
    print(net_g)
    print(net_d)

    d_losses = [] # Empty lists are created to store the loss and accuracy of both the networks
    g_losses = []
    D_x = []
    D_gz = []
    iters = 0
    ...
    '''
    If a previous checkpoint exists, the GAN model is reloaded from that checkpoint.
    '''
    ...
    # The outer loop is for the number of epochs, and the inner loop is for the number of batches.
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for i, (real_data, _) in enumerate(data_loader):
            ...
            if (i+1) % args.n_generator == 0:
                ...
                # The loss is calculated for both the generator and discriminator using the binary cross-entropy loss function.
                ...
                # The generator and discriminator losses are stored in their respective lists.
